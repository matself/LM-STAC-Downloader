...
COL_NAME, COL_COLLECTION, COL_TYPE, COL_YEAR, COL_RES, COL_CHANGED, COL_SIZE = range(7)
...
class StacDock(QDockWidget):
    ...
    def _build_results_group(self) -> QGroupBox:
        ...
        self.tree.setHeaderLabels(["Ruta", "Kollektion", "Typ", "År", "Upplösning", "Ändrad", "Storlek"])
    ...
    def _on_map_clicked(self, point: QgsPointXY, _button) -> None:
        ...
        for key in hits:
            row = self._rows[key]
            parts = [row.text(COL_COLLECTION), row.text(COL_TYPE), row.text(COL_YEAR), row.text(COL_NAME)]
            action = menu.addAction(" · ".join(part for part in parts if part))
